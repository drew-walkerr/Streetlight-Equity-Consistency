...
per_patient_reach_pt_iterative_vpd <- left_join(per_patient_reach_pt_iterative_vpd1,SGLPatientsREDCap, by = "record_id") 
```
...
census_percent_visited = lmer(percent_visited_to_date ~ new_census + days_on_census + ff_clean +(1|record_id), data = per_pt_model_df, REML=FALSE)
...
#Descriptives predicting whether or not pt was visited-- need to refactor FF
descriptives_visit_bin = glmer(visited ~ preferred_gender + project_team + ff + (1|record_id), data = per_pt_model_df, family=binomial("logit"))


# Effect of just new census and days on census

new_census_visit_bin = glmer(visited ~ new_census + days_on_census + (1|record_id), data = per_pt_model_df, family=binomial("logit"))
